...
import { getKofficeClient, clearKofficeClient } from '../services/koffice-client.js';
...
// GET /api/koffice/accounts/:id/resellers/debug - Debug da página de revendas
router.get('/accounts/:id/resellers/debug', async (req, res) => {
  try {
    const { id } = req.params;
    ...
    console.error('Debug koffice resellers error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
