...
// GET /api/gerencia/accounts/:id/users/search - Busca rápida (cache + API)
router.get('/accounts/:id/users/search', withClient, async (req, res) => {
  try {
    ...
    let users = db.searchGerenciaUsersCache(accountId, q, 10);
      ...
      if (result?.props?.users) {
        ...
        if (Array.isArray(apiUsers)) {
          users = apiUsers.map(u => ({
            ...
            mac_device: u.mac_device,
            expire_date: u.expire_date || u.expire_account,
...
router.put('/accounts/:id/users/:userId', withClient, async (req, res) => {
  try {
    ...
    const { serverName, macDevice, m3u8List, expireDate, dns } = req.body;
