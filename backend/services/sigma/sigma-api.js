...
class SigmaAPI {
  ...
  async getServersAndPackages() {
    ...
    for (const server of servers) {
      ...
      for (const pkg of serverPackages) {
        packages.push({
          id: pkg.id,  // Manter como está (pode ser string ou número)
          name: pkg.name,
          server_id: server.id,  // Manter como está (pode ser string ou número)
  ...
  async renewCustomer(customerId, packageId, connections = 1) {
    ...
    const payload = {
      package_id: packageId,  // Manter como está (pode ser string ou número)
  ...
  async createTrialCustomer(serverId, packageId, trialHours, connections = 1) {
    this.log(`Criando cliente trial (servidor ${serverId}, pacote ${packageId})...`);
    
    await this.session.ensureLoggedIn();
    await this.delay(1);
    
    // CORREÇÃO: Não usar parseInt() em IDs que podem ser strings (hashes)
    // A API do Sigma pode usar IDs alfanuméricos
    const payload = {
      server_id: serverId,      // Manter como está (string ou número)
      package_id: packageId,    // Manter como está (string ou número)
